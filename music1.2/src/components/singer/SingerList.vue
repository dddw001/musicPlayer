          ...
          <dt v-for="(items,index) in singerList" class="group" :id="items[0].firstchar">
              ...
              <dl class="group-item">
                  <dt v-for="item in items" @click="open(item)">
                    <img :data-img="item.avatar_small" src="../../assets/img/no.png">
...
import {lazyLoad} from '../../assets/lazy'
import {scrollActive} from '../../assets/scrollActive'
var singerList=[];
export default {
    data(){
        return{
...
.list .group-item dt img{vertical-align: middle;border-radius: 50%;width: 48px;}
