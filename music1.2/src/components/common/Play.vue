<template>
  <div class="play" v-show="songId!=0">
      ...
      <div class="bottom">
          <div class="progress">
              <span class="time time-s">{{Math.floor(currentTime/60)+":"+(currentTime%60/100).toFixed(3).slice(-3,-1)}}</span>
          ...
          <div class="operators">
              <div class="iconfont">
                  <i class="iconfont" :class="loopCur?'icon-danquxunhuan':'icon-xunhuan'" @click="loop"></i>
              </div>
              <div class="iconfont">
                  <i class="iconfont icon-houtui1" @click="prevSong"></i>
              </div>
      ...
      <audio :src="musicInfo.bitrate.file_link" id="audio" autoplay></audio>
  </div>
</template>
...
export default {
  props: ["musicId","searchKey","playList"],
  data() {
    return {
      musicInfo: music,
      currentTime: 0,
  ...
  mounted() {
    ...
    oBtn.onmousedown = function(event) {
      ...
      document.onmousemove = function(e) {
        ...
        if (oBtn.offsetLeft > 0 && l < oBar.offsetWidth) {
          oBtn.style.left = l -8+ "px";
          oRate.style.width = l + "px";
          self.currentTime = l /  oBar.offsetWidth * oAudio.duration;
    ...
    oBtn.ontouchstart = function(event) {
      let disX = event.touches[0].clientX - oBtn.offsetLeft;
  ...
  methods: {
    ...
    play() {
      ...
      if (localStorage.getItem("listenList").length > 0) {
        arr = localStorage.getItem("listenList").split(",");
      } else {
        arr = [];
      }
      if(arr.indexOf(this.songId)==-1){
      ...
      search(this.searchKey).then(res=>{
        ...
        if(this.playList.length){
          ...
          for(let i=0;i<this.playList.length;i++){
            reslut.push({songid:this.playList[i].id});
          }
        }
        for(let i=0;i<reslut.length;i++){
          if(reslut[i].songid==that.songId){
            if(i<reslut.length-1){
              that.songId=reslut[i+1].songid;
            }
            else{
              that.songId=reslut[0].songid;
            }
            break;
          }
        }
      }).then(()=>{
        play(that.songId).then(res => {
        ...
        for(let i=0;i<reslut.length;i++){
          if(reslut[i].songid==that.songId){
            if(i>0){
              ...
              that.songId=reslut[reslut.length-1].songid;
            }
            break;
          }
        }
  ...
  watch: {
    currentTime(val) {
      ...
      oBtn.style.left = (val / oAudio.duration).toFixed(2) * all + "px";
      }
      if(Math.abs(val-this.musicInfo.bitrate.file_duration)<1){
        this.isActive=false;
        if(this.loopCur){
          this.currentTime=0;
          this.isActive=false;
    ...
    playList(arr){
      if(arr){
        this.loopCur=false;
        this.songId=arr[0].id;
        play(this.songId).then(res => {
            //console.log(typeof res.bitrate.file_duration);
            Vue.set(music.songinfo,'pic_small',res.songinfo.pic_small);
            Vue.set(music.songinfo,'author',res.songinfo.author);
            Vue.set(music.songinfo,'title',res.songinfo.title);
  ...
  computed:{
      time(){
          ...
          return Math.floor(d/60)+":"+(d%60/100).toFixed(3).slice(-3,-1)
      }
  }
};
</script>
...
.play .bottom .progress .time-e{margin-left: 8px;}
